...
pub const Shader = struct {
    ...
    pub fn init() !Shader {
        ...
        try checkShader(vert, "vertex");

        const frag = gl.CreateShader(gl.FRAGMENT_SHADER);
        var fs = fs_src;
        gl.ShaderSource(frag, 1, @ptrCast(&fs), null);
        gl.CompileShader(frag);
        try checkShader(frag, "fragment");
...
fn checkShader(shader: gl.GLuint, label: []const u8) !void {
    ...
    if (ok == gl.FALSE) {
        ...
        std.log.err("Zephyr Shader compile failed ({s}): {s}", .{ label, buf[0..@intCast(l)] });
...
fn checkProgram(prog: gl.GLuint) !void {
    ...
    if (ok == gl.FALSE) {
        ...
        std.log.err("Zephyr Program link failed: {s}", .{buf[0..@intCast(l)]});
